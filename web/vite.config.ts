...
function configureProxyRetry(
  proxy: Parameters<NonNullable<Parameters<typeof defineConfig>[0]['server']['proxy'][string]['configure']>>[0],
  path: string,
) {
  proxy.on('error', (err: NodeJS.ErrnoException, req: IncomingMessage, res: ServerResponse) => {
    ...
    const retries = Number((req as any).__retryCount || 0);
    if (retries >= MAX_RETRIES) {
      console.error('\x1b[33m[proxy] Backend still not running after retries. Start with: npm run dev:api\x1b[0m');
    ...
    (req as any).__retryCount = retries + 1;
    console.warn(`\x1b[33m[proxy] Backend not ready, retry ${retries + 1}/${MAX_RETRIES} (${path})\x1b[0m`);
